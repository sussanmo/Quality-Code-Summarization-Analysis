...
def process_df_old(participant, method, start, end, df, fc_dict, dur_dict, scanpath_file):
    scanpath = load_scanpath(scanpath_file, participant, method)
    ...
    for i in range(len(df)):
        ...
        if idx[0].size > 0:
            ...
            if fix_flag and aoi_flag:  # Participant was previously fixating and looking at an AOI
                # Only update if the AOI is part of the scanpath
                if prev_aoi in scanpath:
                    fc_dict[prev_aoi] += 1
    ...
    if fix_flag and aoi_flag and prev_aoi in scanpath:
        fc_dict[prev_aoi] += 1
        dur_dict[prev_aoi].append(time)
...
def main(): 
    ...
    for (p, quality, method), scan_path in scanpath_file.items():
        ...
        if not os.path.isdir(participant_path):
            print(f"Skipping non-directory item: {p}")
            continue
        
        print(f"Processing participant: {p}")
        participant_gaze_files = f"{participant_path}/annotated_gaze"
        
        # List all annotated gaze files
        files = os.listdir(participant_gaze_files) #/315/annotated_gaze
        print(f"Files for participant {p}: {files}")
        # Iterate over the files in the annotated gaze directory
        for file in files:
            filepath = f'{participant_gaze_files}/{file}' 
            print(filepath)
            
            # Load and preprocess data
            df = load_and_preprocess_data(filepath)
            name = filepath.split("_")[-1].replace(".csv", "")
            
            # Determine task based on filename (reading or writing)
            #task = "writing" if "writing" in filepath else "reading"

            # Initialize variables for processing
            dictionaries = init_variables(filepath, df, scanpath_file, p, name) #pass scanpath instead
            bb_start, bb_dict, bb_dur_dict = dictionaries[0], dictionaries[1], dictionaries[2]
            aoi_start, aoi_dict, aoi_dur_dict = dictionaries[3], dictionaries[4], dictionaries[5]

            # Extract the method name (assuming it's part of the file name)
            method_name = name
            ...
            # Prepare the final output for duration (fixation count is commented out)
            final_dur = {"pid": p, "quality": quality, "method": method}
    ...
    # Save the final results to a pickle file
    with open('scan_paths_nonaggregate_fixDuration.pkl', 'wb') as f:
        pickle.dump(scanpath_fixation_map, f)
...
def main2():
    ...
    #for p in participants:
    for (p, quality, method), scan_path in scanpath_file.items():
        participant_path = f"{superpath}/{p}"
        if not os.path.isdir(participant_path):  # Skip if it's not a directory
            print(f"Skipping non-directory item: {p}")
            continue

        print(f"Processing participant: {p}")
        files = os.listdir(f"{participant_path}/annotated_gaze")
        ...
        for file in files:
            ...
            # Determine task (reading or writing) based on file name
            #task = "writing" if "writing" in filepath else "reading"

            # Initialize variables
            dictionaries = init_variables(filepath, df, scanpath_file, p, name)
            
            bb_start, bb_dict, bb_dur_dict = dictionaries[0], dictionaries[1], dictionaries[2]
            aoi_start, aoi_dict, aoi_dur_dict = dictionaries[3], dictionaries[4], dictionaries[5]

            # Extract the method name for scanpath loading
            method_name = name  # Assuming name represents the method
            ...
            # Calculate the mean fixation durations
            mean_aoi_durations = calculate_mean_fixation(aoi_dur_dict)
            mean_bb_durations = calculate_mean_fixation(bb_dur_dict)
            ...
            # Prepare final output dictionaries for fixation count and duration
            #final_fix = {"pid": p}
            final_dur = {"pid": p, "quality": quality, "method": method_name}
            
            #final_fix.update(bb_dict)
            final_dur.update(mean_bb_durations)
            
            # Convert numpy types to Python native types for both final dictionaries
            #final_fix_clean = convert_np_to_python(final_fix)
            final_dur_clean = convert_np_to_python(final_dur)
    ...
    with open('scan_paths_nonaggregate_fixDuration.pkl', 'wb') as f:
        pickle.dump(scanpath_fixation_map, f)
            #print(final_dur)
            # Save results to task-specific and combined CSVs
